...
### 3.3 표(Table) 작성 시 컬럼-내용 일치

- 표의 각 열 제목(헤더)이 정한 종류의 내용만 그 열에 채운다. 특히 헤더가 **"태그"/"태그·속성"**인 열에는 반드시 실제 HTML 태그나 속성(예: `href="#id"`, `colspan`)을 백틱으로 감싸 적는다. "목차 기능"처럼 기능 설명 문구를 태그 칸에 대신 적지 않는다 — 설명은 옆 칸("의미"/"핵심 개념" 등)에 쓴다.
- 표를 다 쓴 뒤, 태그/속성 열에 백틱(`` ` ``)이 빠진 행이 있는지 한 번 훑어서 확인한다.

### 3.4 기술 용어 정확성

- HTML 태그명, CSS 속성명, 명령어처럼 철자가 정확해야 하는 용어는 지어내거나 줄여 쓰지 않고, 아는 그대로 정확히 적는다 (예: "마우스"를 "마유수"처럼 잘못 쓰지 않기).
- 초안을 다 쓴 뒤, 본문에 나온 태그명·속성명·명령어를 한 번 더 읽으며 실제 존재하는 정확한 이름인지 확인한다. 특히 상품명·사물 이름처럼 반복 등장하는 단어(카드 목록, 예시 데이터 등)는 항목마다 철자가 같은지 대조한다.

### 3.5 문장 다듬기 (자체 교정)

- 초안 작성 후, 처음부터 끝까지 다시 읽으며 어색하거나 문법이 안 맞는 문장이 없는지 확인한다. 특히 코드/HTML 안에 넣는 한글 텍스트(예: `<span>내용</span>`)는 띄어쓰기가 빠지거나 붙어버리기 쉬우니 별도로 확인한다.
- 확신이 서지 않는 표현은 더 단순하고 명확한 문장으로 바꾼다.
...
- [ ] 표의 "태그/속성" 열에 실제 태그·속성만 있고, 기능 설명 문구가 잘못 들어가진 않았는가?
- [ ] 태그명·속성명·명령어 등 철자가 정확한가 (다시 한 번 눈으로 대조)?
- [ ] 처음부터 끝까지 다시 읽어 어색한 문장이나 띄어쓰기 오류가 없는가?