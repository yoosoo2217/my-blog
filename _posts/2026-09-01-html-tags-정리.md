...
`strong`/`b`, `em`/`i`처럼 눈에 보이는 결과는 비슷해도 **의미가 있는 태그(strong, em)와 순수 스타일 태그(b, i)**가 나뉜다는 걸 배웠다.
...
| `ol` + `li` | 순서 있는 목록 (번호) |
| `ol`의 `type` 속성 | 번호 표기 방식 지정 (`1`, `a`, `A`, `i`, `I` 등) |
...
`ul`과 `ol` 중 뭘 쓸지는 겉모습(점 vs 숫자)이 아니라 **항목 사이에 순서가 의미를 갖는지**로 정해야 한다는 걸 배웠다. "좋아하는 것"처럼 나열 순서가 상관없으면 `ul`, "조리 순서"처럼 순서 자체가 정보면 `ol`을 쓴다.
...
`colspan`과 `rowspan`은 처음엔 헷갈렸는데, col(열)을 합친다는 뜻의 span이 가로 병합이고, row(행)을 합친다는 뜻의 span이 세로 병합이라는 뜻으로 외우니 헷갈리지 않았다.
...
`alt`는 이미지를 꾸미는 옵션이 아니라, 이미지가 로드되지 않거나 스크린리더가 페이지를 읽을 때 **이미지 대신 전달되는 실제 내용물**이다. 그래서 "사진"처럼 의미 없는 문구가 아니라 무엇을 찍었는지 알 수 있는 문구를 써야 한다는 점을 새겨뒀다.
...
`controls` 속성 하나만 붙이면 재생/정지/음량 같은 UI를 브라우저가 알아서 그려준다는 점이 인상 깊었다. 직접 버튼을 만들지 않아도 되니, 커스텀 UI가 필요 없는 경우엔 이 기본 컨트롤만으로 충분하다.
...
| `href="#id값"` | 페이지 안의 특정 위치(목차)로 바로 이동 |
